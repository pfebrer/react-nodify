...
import { worker } from './pyodide_worker';
...
class PythonApi {
    ...
    nodeToPythonScript = async (name, includeDefaults, asFunction, functionName) => {
        return this._sessionMethod("node_to_python_script", { key: name, include_defaults: includeDefaults || false, as_function: asFunction || false, function_name: functionName || null})
    }
...
class PyodidePythonApi extends PythonApi {
    ...
    _init_worker = () => {
        ...
        var blobURL = URL.createObjectURL( new Blob([ '(', worker.toString() ,')()' ], { type: 'application/javascript' } ) )

        this.pyodide_worker = new Worker( blobURL );
    ...
    loadPackages = async (packages) => {
        ...
        return new Promise((res, rej) => {
            ...
            this.pyodide_worker.postMessage({type: "loadPackages", packages}, [channel.port2])
        });

    }

    micropipInstall = async (packages, keep_going, deps) => {
        ...
        return new Promise((res, rej) => {
            ...
            this.pyodide_worker.postMessage({type: "micropipInstall", packages, keep_going, deps}, [channel.port2])
        });
